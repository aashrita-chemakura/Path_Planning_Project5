...
class ImprovedRRTStar(RRTStar):
    def __init__(self, start: tuple, goal: tuple, env: Map, max_dist: float,
                 sample_num: int, r: float, goal_sample_rate: float = 0.05,c_best=float("inf")) -> None:
    ...
    def plan(self):
        ...
        if node_new:
            ...
            # goal found
            if dist <= self.max_dist and not self.collsion(node_new, self.goal):
    ...
    def run(self) -> None:
        ...
        for i in range(self.sample_num):
            cost, path = self.plan()
            self.adaptive_factor=(i/self.sample_num)*self.c_best
            # update
            if path and cost < best_cost:
                print("Updated Cost",cost)
                self.c_best = best_cost = cost
                self.updated_c=self.c_best
                best_path = path
            # animation
            if  i % 30 == 0:
    ...
    def generateRandomNode(self) -> Node:
        ...
        if self.updated_c <= self.c_best:
            while True:
                ...
                p_star = self.transform(self.updated_c/ 2) @ p.T
    ...
    def drawEllipse(self):
        ...
        z = [1 for _ in t]
        fx = self.transform(self.updated_c/ 2) @ np.array([x, y, z])
